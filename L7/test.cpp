...
TEST_CASE("cache_set_find_line_matching::RANDOMIZED_MARKING",
          "[weight=1][part=test]") {
  simcache_t cache;
  ...
  cache_line_t lines[] = {{true, false, 0, 0, 10},
                          {true, false, 0, 0, 11},
                          {true, false, 0, 0, 12},
                          {false, false, 0, 0, 13}};
...
TEST_CASE("cache_set_find_line_matching::LFU", "[weight=1][part=test]") {
  ...
  cache_set_t cache_set;
  cache_line_t lines[] = {{true, false, 6, 1, 10},
                          {false, false, 0, 0, 13},
                          {true, false, 5, 1, 11},
                          {true, false, 5, 1, 12}};
...
TEST_CASE("find_available_cache_line::RANDOMIZED_MARKING",
          "[weight=1][part=test]") {
  simcache_t cache;
  cache.policies = CACHE_REPLACEMENTPOLICY_RANDOMIZED_MARKING;
  cache.num_lines = 16;
  ...
  // For testing purposes, we are mocking "generate_random_number" to always
  // return 1 If you are really measuring the miss rate of your cache, use
  // "rand" (from stdlib.h) instead of "[](){ return 1; }"
  cache_line_t *actual =
      find_available_cache_line(&cache, &cache_set, []() { return 1; });
  ASSERT_EQUAL(actual, &lines[2]);
  ASSERT_EQUAL(lru_list[0], 3);
  ASSERT_EQUAL(lru_list[1], 2);
  ASSERT_EQUAL(lru_list[2], 1);
...
TEST_CASE("cache_new,lfu_cache_line_setup,cache_free",
          "[weight=1][part=test]") {
  simcache_t *cache = cache_new(16, 2, 4, CACHE_REPLACEMENTPOLICY_LFU);
  ASSERT_EQUAL(1, 1);
}